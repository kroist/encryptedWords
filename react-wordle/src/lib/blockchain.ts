...
import { wordAtIdToNumber, numberToWord } from "./words";
...
const getGameAddr = async () => {
    ...
    const contractAddr = await factoryContract.userLastContract(await signer.getAddress());
    return contractAddr;
}
...
export const startNewGame = async () => {
    ...
    return factoryContract.createGame(await signer.getAddress(), {
        gasLimit: 4000000,
    })
    .then((tx:any) => {
      return tx.wait();
...
export const callRelayerForWord = async (metamaskProvider: SDKProvider) => {
...

    {
        let tx = await contract[
            "submitWord1(bytes,bytes,bytes,bytes,bytes,bytes)"
        ](
...
export const getGuesses = async (prevGuesses:[string,number,number][]) => {
  ...
  for (let i = prevGuesses.length; i < guessesNumber; i++) {
    const [eqMaskGuessBytes, letterMaskGuessBytes] = await contract.getGuess(i, params.publicKey, params.signature);
    const eqMaskGuess: number = fhevmInstance.decrypt(contractAddress, eqMaskGuessBytes);
    const letterMaskGuess: number = fhevmInstance.decrypt(contractAddress, letterMaskGuessBytes);
    let word = 0;
    for (let j = 4; j >= 0; j--) {
      const letterIEnc = await contract.getLetterGuess(i, j, params.publicKey, params.signature);
...
export const claimWin = async (guessN: number) => {
  let contract = await getGameContract();
  return contract.claimWin(guessN, {
      gasLimit: 4000000,
  }).then((tx:any) => {
    return tx.wait();
  }).then(() => {
